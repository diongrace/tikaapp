...
def process_file(filepath):
    ...
    if FA_IMPORT not in content and 'Icons.' in content:
        content = content.replace(MATERIAL_IMPORT, MATERIAL_IMPORT + '\n' + FA_IMPORT, 1)

    sorted_keys = sorted(MAPPING.keys(), key=len, reverse=True)
    for mat_name in sorted_keys:
        fa_name = MAPPING[mat_name]
        content = content.replace('Icons.' + mat_name, 'FontAwesomeIcons.' + fa_name)
...
lib_path = 'C:/src/Tika/tikaapp/lib'
changed = 0
total = 0
for root, dirs, files in os.walk(lib_path):
    for fname in files:
        if fname.endswith('.dart'):
            ...
            if 'Icons.' in open(fpath, encoding='utf-8').read():
                total += 1
                if process_file(fpath):
                    print('OK: ' + fpath)
                    changed += 1

print('Done: ' + str(changed) + '/' + str(total) + ' files changed')
