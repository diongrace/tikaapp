...
foreach ($file in $files) {
    ...
    # Multiline: replace Icon( followed by optional whitespace/newline then FontAwesomeIcons
    # But NOT FaIcon( already
    $new = [regex]::Replace($content, '(?<!Fa)Icon\((\s*FontAwesomeIcons)', 'FaIcon($1')
    if ($new -ne $content) {
